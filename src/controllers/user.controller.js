...
const verifyUser = asyncHandler(async (req, res) => {
  ...
  const isCodeValid =
    user.verifyCodeExpiry && user.verifyCodeExpiry > Date.now();
  if (!isCodeValid) {
...
const googleLink = asyncHandler(async (req, res) => {
  try {
    ...
    if (!email) {
      ...
      console.error(
        "googleLink: Email not found in session. Session might be expired or not set."
      );
      throw new APIError(
        401,
        "Session data missing for Google linking. Please try registering again."
      );
    }
    ...
    // CSRF State verification
      ...
      if (!user) {
        ...
        throw new APIError(
          404,
          "User not found in database for Google linking email."
        );
      }
      ...
      try {
        ...
        throw new APIError(
          500,
          "User Registered Successfully but failed to Send Invite Code email"
        );
      }
...
const primaryAndSecondaryLink = asyncHandler(async (req, res) => {
  ...
  if (!tempToken) {
    throw new APIError(
      404,
      "No temp cookie found for linking primary and secondary users."
    );
  }
  const decodeToken = jwt.verify(tempToken, process.env.TEMP_TOKEN_SECRET);
  ...
  const isCodeValid =
    primaryUser.inviteCodeExpiry && primaryUser.inviteCodeExpiry > Date.now();
  if (!isCodeValid) {
  ...
  try {
    ...
    throw new APIError(
      500,
      "Users linked but failed to Success for Primary User email"
    );
  }

  // Send Success for Secondary User email
  try {
    await sendSecondarySuccessEmail(
      secondaryUserEmail,
      secondaryUserName,
      primaryUserName
    );
    ...
    throw new APIError(
      500,
      "Users linked but failed to send Success for Secondary User email"
    );
  }
...
const rolecheck = asyncHandler(async (req, res) => {
  ...
  if (user.role === "Primary" || user.role === "Secondary") {
    return res
      .status(200)
      .json(new APIResponse(200, { role: user.role }, "Role Check Successful"));
  }
  ...
  return res
    ...
    .json(
      new APIResponse(
        405,
        {},
        "Unauthorized | Invalid Account | Suspicious Account"
      )
    );
});
